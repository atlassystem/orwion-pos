...
function buildReportHtml(
  ...
  const maliTablo =
    s.orderCount > 0 && d.vat.length
      ? `<h2>Mali Fiş Özeti (KDV)</h2>
         <table class="grid">
           <thead><tr><th>KDV Oranı</th><th class="num">Matrah</th><th class="num">KDV</th><th class="num">Tutar</th></tr></thead>
           <tbody>${d.vat
             .map(
               (v) =>
                 `<tr><td>%${v.rate}</td><td class="num">${esc(TL(v.base))}</td><td class="num">${esc(TL(v.kdv))}</td><td class="num">${esc(TL(v.total))}</td></tr>`,
             )
             .join("")}</tbody>
         </table>
         ${
           d.fiscal.length
             ? `<p class="empty">Fiş durumu: ${d.fiscal
                 .map((f) => `${esc(fiscalLabel(f.status))} (${f.count})`)
                 .join(" · ")}</p>`
             : ""
         }`
      : "";
      ...
      <div className="scroll-light space-y-4 overflow-y-auto px-7 pb-7">
        ...
        {s.orderCount > 0 && (
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            {/* Mali fiş özeti — KDV kırılımı (ÖKC hazırlığı) */}
            {d.vat.length > 0 && (
              <Card title="Mali Fiş Özeti (KDV)" icon={Receipt} className="lg:col-span-2">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-[11px] font-bold tracking-wide text-ink3 uppercase">
                        <th className="pb-2">KDV Oranı</th>
                        <th className="pb-2 text-right">Matrah</th>
                    ...
                    <tbody>
                      {d.vat.map((v) => (
                        <tr key={v.rate} className="border-t border-line">
                          <td className="py-2 font-semibold text-ink">%{v.rate}</td>
                          <td className="py-2 text-right tnum text-ink2">{TL(v.base)}</td>
                          <td className="py-2 text-right tnum font-semibold text-ink">
                            {TL(v.kdv)}
                          </td>
                          <td className="py-2 text-right tnum text-ink2">{TL(v.total)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {d.fiscal.length > 0 && (
                  <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-dashed border-line2 pt-3 text-[12px]">
                    <span className="font-semibold text-ink3">Fiş Durumu:</span>
                    {d.fiscal.map((f) => (
                      <span
                        key={f.status}
                        className="inline-flex items-center gap-1 rounded-full bg-surface2 px-2.5 py-1 font-bold text-ink2"
                      >
                        {fiscalLabel(f.status)}
                        <span className="text-ink3">({f.count})</span>
                      </span>
                    ))}
                  </div>
                )}
